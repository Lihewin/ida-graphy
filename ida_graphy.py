...
Commands:
  ...
  project verify-exports <name>         Verify DB export paths against manifests
...
from exporters.export_manifest import verify_project_export_paths
...
def cmd_project_verify_exports(args, config: Dict) -> int:
    """快速校验数据库路径、manifest 和导出文件是否一致。"""
    try:
        project_manager = ProjectManager(config['projects']['root_dir'])
        project = project_manager.get_project(args.name)
        project_dir = str(project_manager._get_project_dir(project.name))
        db_path = os.path.join(project_dir, project.graph_db_file)

        if not os.path.exists(db_path):
            logger.error(f"LadybugDB 不存在: {db_path}")
            return 1

        result = verify_project_export_paths(project_dir, db_path)
        if args.output_format == "json":
            print(json.dumps(result, ensure_ascii=False, indent=2, default=str))
            return 0 if result.get("ok") else 1

        print(f"导出路径校验: 检查 {result.get('checked', 0)} 条路径")
        if result.get("ok"):
            print("✅ 导出路径、manifest 和磁盘文件一致")
            return 0

        print(f"❌ 发现 {result.get('issue_count', 0)} 个问题:")
        for issue in result.get("issues", [])[:50]:
            print(
                "  - "
                f"{issue.get('owner_type')} {issue.get('owner_id')} "
                f"{issue.get('path')}: {issue.get('message')}"
            )
        if result.get("issue_count", 0) > 50:
            print(f"  ... 另有 {result.get('issue_count', 0) - 50} 个问题")
        return 1

    except ProjectError as e:
        logger.error(f"校验项目导出路径失败: {e}")
        return 1
    except LadybugDBError as e:
        logger.error(f"LadybugDB 校验失败: {e}")
        return 1
    except Exception as e:
        logger.error(f"校验项目导出路径失败: {e}")
        return 1
...
示例用法:
  ...
  ida-graphy project status malware_analysis
    ...
    # project verify-exports
    verify_exports_parser = project_subparsers.add_parser('verify-exports', help='快速校验导出路径')
    verify_exports_parser.add_argument('name', help='项目名称')
    verify_exports_parser.add_argument('--output-format', '-f', choices=['table', 'json'], default='table', help='输出格式')
...
def main() -> int:
    ...
    try:
        ...
        if args.command == 'project':
            ...
            elif args.project_action == 'verify-exports':
                return cmd_project_verify_exports(args, config)
            else:
                parser.print_help()
                return 1
