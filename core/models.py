...
class BinaryNode:
    ...
    Attributes:
        ...
        export_manifest_file: Project-relative path to the export manifest
        export_manifest_hash: SHA-256 of exports/<binary>/_export_manifest.json
    """
    hash: str
    name: str
    base_addr: int
...
class GraphData:
    ...
    def node_count(self):
        """返回总节点数"""
        return len(self.binaries) + len(self.functions) + len(self.dataslots) + len(self.strings)
    ...
    def merge(self, other: 'GraphData') -> None:
        ...
        """
        # 合并节点（避免重复）
        existing_binaries = {b.hash: b for b in self.binaries}
        ...
        for binary in other.binaries:
            existing_binary = existing_binaries.get(binary.hash)
            if existing_binary is None:
                self.binaries.append(binary)
                existing_binaries[binary.hash] = binary
            else:
                if binary.export_manifest_file:
                    existing_binary.export_manifest_file = binary.export_manifest_file
                if binary.export_manifest_hash:
                    existing_binary.export_manifest_hash = binary.export_manifest_hash
